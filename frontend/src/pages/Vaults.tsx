...
export default function Vaults() {
  ...
  const filtered = VAULTS.filter((v) => {
    if (filter === 'Base Vaults') return v.id === 'sbtc-base';
    if (filter === 'Auto-Compound') return v.id === 'sbtc-auto';
    return true;
  });

  const getVaultType = (id: string) => id === 'sbtc-auto' ? 'Auto-Compound' : 'Base';
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
          {filtered.map((vault) => {
            const utilization = (vault.totalDeposited / vault.depositCap) * 100;
            const vaultType = getVaultType(vault.id);
            return (
              <Link
                key={vault.id}
                to={`/vaults/${vault.id}`}
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-right">
                      <span className="font-mono text-lg font-semibold text-success">{vault.apy.toFixed(2)}%</span>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    ...
                    <p className="font-mono text-sm text-foreground">{vault.tvl.toFixed(3)} <span className="text-muted-foreground">sBTC</span></p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Share Price</p>
                    <p className="font-mono text-sm text-foreground">{vault.sharePrice.toFixed(4)}</p>
                  </div>
                </div>
